...
function Navbar() {
  ...
  return (
    <div className="fixed top-0 w-full z-20 pt-5 ">
      <div className="max-w-[1322px] mx-auto flex justify-between items-center px-4 py-4 lg:py-6">
        <img src={logo} alt="Logo" className="w-[153px] h-[27px] object-cover" />
        <ul className="hidden lg:flex flex-row gap-[60px] text-[15px] font-medium text-white">
          {links}
        </ul>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <ul className="flex flex-col gap-4 bg-amber-950 text-white p-4 text-[15px] font-medium lg:hidden">
          {links}
        </ul>
      )}
    </div>
